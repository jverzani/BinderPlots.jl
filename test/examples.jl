## This is https://raw.githubusercontent.com/JuliaPlots/Plots.jl/master/src/examples.jl
## with tests for BinderPlots
## run
## filter(kv -> !isnothing(last(kv)), binder_plots_test())
## to generate dictionary of passing examples (23 of them)
"""
...
_backend_skips = Dict(
    ...
    :gaston => [
        ...
        63,  # FIXME: twin axes misalignment
    ],
)
...
function _binder_plots_test(eg)
    pkgname = :BinderPlots
    m = Module(Symbol(:PlotsExamples, pkgname))
    ...
    Base.eval(m, quote
              ...
              using BinderPlots
              import Plots
              RGB(as...) = rgb(round.(Int, 255 .* as)...)
              RGBA(a,b,c,α) = rgb(round.(Int, 255 .* (a,b,c))..., α)
              #rng = $rng
              #rng === nothing || Random.seed!(rng, Plots.PLOTS_SEED)
              #=
              theme(:default)
    ...
    (imp = eg.imports) === nothing || Base.eval(m, imp)
    exprs = eg.exprs
    #rng === nothing || (exprs = Plots.replace_rand(exprs))
    Base.eval(m, exprs)
    #m.BinderPlots.current()
end

function binder_plots_test()
    d = Dict{Any,Any}()
    for (i,eg) ∈ enumerate(_examples)
        p = try
            _binder_plots_test(eg)
        catch err
            nothing
        end
        d[i] = p
